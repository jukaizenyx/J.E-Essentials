...
$je_notifications = function_exists('je_get_notifications') ? je_get_notifications((int)$je_user['id']) : [];
$je_has_unread_notifications = false;

foreach ($je_notifications as $notification) {
    if ((int)$notification['is_read'] === 0) {
        $je_has_unread_notifications = true;
        break;
    }
}
?>
        ...
        <div class="orders-list">
            ...
            <article class="order-card" data-order-id="<?= (int)$order['id'] ?>" data-status="<?= htmlspecialchars($status) ?>">
                <header class="order-card__head">
                    ...
                    <span class="order-status order-status--<?= htmlspecialchars($status_class) ?>"><?= htmlspecialchars($status_label) ?></span>
                </header>
                ...
                <footer class="order-card__foot">
                    ...
                    <button type="button" class="btn-cancel-order" data-order-id="<?= (int)$order['id'] ?>" <?= $can_cancel ? '' : 'disabled' ?>><?= $status === 'Cancelled' ? 'Cancelled' : 'Cancel order' ?></button>
                </footer>
            </article>
            <?php endforeach; ?>
        </div>
...
<?php
if ($je_has_unread_notifications && function_exists('je_mark_notifications_read')) {
    je_mark_notifications_read((int)$je_user['id']);
}
?>
</body>
</html>
